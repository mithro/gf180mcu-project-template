...
def emit_pad_cfg(cfg: ExactConfig, placements: dict) -> str:
    ...
    *byte-identical* origin.  Only the 2 relocated analog pads use an
    ...
    a("")
    a("# Extend the adjacent IO row over the freed corner X strip by manually")
    a("# placing IO filler cells at the row's Y origin / orientation. This makes")
    a("# the N/S filler band run all the way to the die edge on a bare side")
    a("# rather than stopping ~355um short where the corner used to sit. The new")
    a("# fillers abut the row's last existing cell (same Y + orient + adjacent X)")
    a("# so they extend the power-abutment chain naturally for connect_by_abutment.")
    a("set _db [ord::get_db]")
    a("# Filler masters sorted by width (DB units) descending for greedy packing.")
    a("set _filler_choices [list]")
    a("foreach _fname $::env(PAD_FILLERS) {")
    a("    set _fm [$_db findMaster $_fname]")
    a("    if {$_fm == \"NULL\"} { continue }")
    a("    lappend _filler_choices [list [$_fm getWidth] $_fname $_fm]")
    a("}")
    a("set _filler_choices [lsort -decreasing -integer -index 0 $_filler_choices]")
    a("set _bare_fill_count 0")
    ...
    a("    set _row_name [expr {$_is_north ? \"IO_NORTH\" : \"IO_SOUTH\"}]")
    a("    if {![dict exists $_rows_by_name $_row_name]} { continue }")
    a("    set _row [dict get $_rows_by_name $_row_name]")
    a("    # getBBox is the well-supported API across odb bindings; the row's")
    a("    # yMin is the Y origin of its sites (cells are placed at this Y).")
    a("    set _row_bb [$_row getBBox]")
    a("    set _row_y [$_row_bb yMin]")
    a("    set _row_orient [$_row getOrient]")
    a('    puts "\\[INFO\\]   ext row=$_row_name y=$_row_y orient=$_row_orient '
      'x=[$_row_bb xMin]-[$_row_bb xMax] corner_x=${_cxlo}-${_cxhi}"')
    a("    set _x $_cxlo")
    a("    while {$_x < $_cxhi} {")
    a("        set _rem [expr {$_cxhi - $_x}]")
    a("        set _placed 0")
    a("        foreach _fc $_filler_choices {")
    a("            lassign $_fc _fwidth _fname _fmaster")
    a("            if {$_fwidth <= $_rem} {")
    a("                incr _bare_fill_count")
    a("                set _iname \"_bare_fill_$_bare_fill_count\"")
    a("                set _new [odb::dbInst_create $block $_fmaster $_iname]")
    a("                $_new setOrient $_row_orient")
    a("                $_new setLocation $_x $_row_y")
    a("                $_new setPlacementStatus FIRM")
    a("                set _x [expr {$_x + $_fwidth}]")
    a("                set _placed 1")
    a("                break")
    a("            }")
    a("        }")
    ...
    a("    }")
    a("}")
    a('puts "\\[INFO\\] Bare-edge corner fill: placed $_bare_fill_count extension filler(s)."')
    a("")
