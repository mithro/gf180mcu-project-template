...
def emit_pad_cfg(cfg: ExactConfig, placements: dict) -> str:
    ...
    *byte-identical* origin.  Only the 2 relocated analog pads use an
    ...
    a("")
    a("# Extend the adjacent IO row over the freed corner X strip by creating a")
    a("# SUPPLEMENTAL dbRow at the corner location with the same site / orient /")
    a("# direction as the original row, then letting `place_io_fill` populate it.")
    a("# This is preferable to dbInst_create'ing fillers manually because:")
    a("#  - `place_io_fill` produces site-aligned cells the row-walking power")
    a("#    chain understands;")
    a("#  - `connect_by_abutment` (called below) walks dbRows by name and would")
    a("#    miss standalone cells outside any row, leading to Magic Illegal")
    a("#    Overlap errors at DRC time (~357-1430 per build observed).")
    a("# We name the new rows IO_<side>_EXT_<x> so they look like ordinary IO rows")
    a("# to place_io_fill but don't collide with the original row name.")
    a("set _ext_row_names [list]")
    a("# Pre-iterate rows once; dbBlock has no findRow, just getRows.")
    a("set _rows_by_name [dict create]")
    a("foreach _r [$block getRows] {")
    a("    dict set _rows_by_name [$_r getName] $_r")
    a("}")
    a("foreach _data $_bare_corner_data {")
    a("    lassign $_data _cxlo _cylo _cxhi _cyhi _is_north _inst")
    a("    set _is_east [expr {(($_cxlo + $_cxhi) / 2.0 / $_units) > $_xmid}]")
    a("    # Decide which adjacent row of this destroyed corner is KEPT (not bare).")
    a("    # NS side is the corner's north/south side; EW side is its east/west side.")
    ...
    a("    }")
    a("    # DIAGNOSTIC: skip IO_NORTH extensions. CI shows IO_SOUTH ext rows")
    a("    # work cleanly (0p5x0p5_2side_exact SE corner passed DRC) but adding")
    a("    # IO_NORTH ext rows causes 357 Magic Illegal Overlap errors. Isolating.")
    a("    if {$_row_name eq \"IO_NORTH\"} {")
    a('        puts "\\[INFO\\] DIAGNOSTIC: skipping IO_NORTH extension at corner '
      'x=${_cxlo}-${_cxhi} -- being investigated"')
    a("        continue")
    a("    }")
    ...
    a("}")
    a('puts "\\[INFO\\] Bare-edge corner fill: created [llength $_ext_row_names] '
      'extension row(s) to fill via place_io_fill."')
    a("")
    ...
    a("}")
    a("# Fill the corner-extension rows we created above. Doing this AFTER the")
    a("# main rows are filled means the existing IO_NORTH/IO_SOUTH cells are now")
    a("# in place at the corner-adjacent X, so place_io_fill's adjacency-aware")
    a("# packing knows where to start (just east of the row's last filler).")
    a("foreach _ext $_ext_row_names {")
    a('    puts "\\[INFO\\] Filling extension row $_ext"')
    a("    place_io_fill -row $_ext {*}$::env(PAD_FILLERS)")
    a("}")
    a("")
