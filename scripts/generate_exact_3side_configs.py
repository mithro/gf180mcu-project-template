...
class ExactConfig:
    ...
    cut: str             # bare-cut edge: one of S/N/E/W
...
def emit_yaml(cfg: ExactConfig, placements: dict) -> str:
    lines = []
    a = lines.append
    a("# Exact-pad 3-side configuration -- AUTOGENERATED by")
    a("# scripts/generate_exact_3side_configs.py.  DO NOT EDIT BY HAND.")
    a("#")
    a(f"# Half-size slot_1x1 with the {cfg.cut} edge as a bare cut edge.")
    ...
    a("")
    a("# Partial padring (3-sided): use the partial PDN script.")
...
def emit_defines_block(cfg: ExactConfig, counts: dict) -> str:
    ...
    return "\n".join([
        f"`ifdef {cfg.define}",
        "",
        f"// Exact-pad half slot_1x1 ({cfg.cut} edge = bare cut).",
...
def main() -> int:
    ...
    for cfg in CONFIGS:
        ...
        summary.append(
            f"{cfg.name}: die={cfg.die} core={cfg.core} cut={cfg.cut} "
