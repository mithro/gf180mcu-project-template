# Slot Usable Area Documentation Generator - Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Generate comprehensive slot documentation showing usable area, IO counts, and validation across data sources, deployed to GitHub Pages.

**Architecture:** A Python script parses slot YAML files to extract CORE_AREA (usable area) and IO counts, validates against pad geometry from LEF files, and generates JSON/Markdown/HTML outputs for GitHub Pages deployment.

**Tech Stack:** Python 3.11+, PyYAML, Pillow, klayout (optional), uv for running

---

## Task 1: Create SlotInfo Data Model

**Files:**
- Create: `scripts/generate_slot_docs.py`

**Step 1: Create the script with dataclass and basic structure**
...
Run: `uv run scripts/generate_slot_docs.py`
Expected: "SlotInfo dataclass created successfully"

**Step 3: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add SlotInfo dataclass for slot documentation"
```

---

## Task 2: Parse Slot YAML Files

**Files:**
- Modify: `scripts/generate_slot_docs.py`

**Step 1: Add YAML parsing function**

Add after `SLOT_LABELS` dictionary:
...
**Step 2: Update main to test parsing**
...
Run: `uv run scripts/generate_slot_docs.py`
Expected output showing all 4 slots with dimensions and IO counts

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add YAML parsing for slot configurations"
```

---

## Task 3: Add JSON Output Generation
...
Add after `load_all_slots` function:
...
**Step 2: Update main to test JSON generation**
...
**Step 3: Run and verify JSON output**

Run: `uv run scripts/generate_slot_docs.py && cat gh-pages/slots.json`
Expected: Valid JSON with all 4 slots

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add JSON output generation for slots"
```

---

## Task 4: Add Markdown Output Generation

**Files:**
- Modify: `scripts/generate_slot_docs.py`

**Step 1: Add Markdown generation function**

Add after `generate_json` function:
...
**Step 2: Update main to generate both outputs**
...
**Step 3: Run and verify Markdown output**

Run: `uv run scripts/generate_slot_docs.py && cat gh-pages/SLOTS.md`
Expected: Formatted Markdown with tables

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add Markdown output generation for slots"
```

---

## Task 5: Add HTML Output Generation

**Files:**
- Modify: `scripts/generate_slot_docs.py`

**Step 1: Add HTML generation function**

Add after `generate_markdown` function:
...
**Step 2: Update main to generate all outputs**
...
**Step 3: Run and verify HTML output**

Run: `uv run scripts/generate_slot_docs.py && ls -la gh-pages/`
Expected: slots.json, SLOTS.md, index.html all present

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add HTML output generation for GitHub Pages"
```
...
**Step 1: Replace main block with proper CLI**
...
**Step 2: Test CLI with arguments**

Run: `uv run scripts/generate_slot_docs.py --help`
Expected: Help message with options

Run: `uv run scripts/generate_slot_docs.py -o test-output`
Expected: Files generated in test-output/
...
## Task 7: Add Pad Geometry Validation

**Files:**
- Modify: `scripts/generate_slot_docs.py`

**Step 1: Add LEF parsing for pad dimensions**
...
**Step 2: Integrate validation into main**

Update the `main()` function, add after loading slots:
...
**Step 3: Run and verify validation**

Run: `uv run scripts/generate_slot_docs.py`
Expected: Validation messages (pass or warnings)

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add pad geometry validation"
```

---

## Task 8: Add Image Download Integration

**Files:**
- Modify: `scripts/generate_slot_docs.py`

**Step 1: Add image download functionality**
...
**Step 2: Add CLI flag for image download**

Update argument parser in `main()`:
...
Update main() after slots loading, before generating outputs:
...
**Step 3: Test without downloading (no gh CLI needed)**

Run: `uv run scripts/generate_slot_docs.py`
Expected: Normal output without image download

**Step 4: Commit**

```bash
git add scripts/generate_slot_docs.py
git commit -m "feat: add image download from GitHub Actions"
```

---

## Task 9: Add Makefile Target
...
Check structure of existing Makefile to add target appropriately.

**Step 2: Add slot-docs target**

Add near end of Makefile before .PHONY declarations:
...
.PHONY: slot-docs slot-docs-with-images
```

**Step 3: Test Makefile target**

Run: `make slot-docs`
Expected: Documentation generated in gh-pages/

**Step 4: Commit**

```bash
git add Makefile
git commit -m "feat: add Makefile targets for slot documentation"
```
...
**Files:**
- Clean: `gh-pages/` (remove test outputs)

**Step 1: Clean test outputs**
...
Run: `make slot-docs`
Expected: Clean generation with all outputs

**Step 3: Verify outputs**

```bash
cat gh-pages/slots.json | head -30
cat gh-pages/SLOTS.md
```
...
git commit -m "chore: clean up test outputs"
```
...
1. **`scripts/generate_slot_docs.py`** - Main script with:
   - SlotInfo dataclass
   - YAML parsing for slot configs
...
2. **Makefile targets**:
   - `make slot-docs` - Generate docs without images
   - `make slot-docs-with-images` - Generate docs with CI images

3. **Outputs in `gh-pages/`**:
   - `slots.json` - Machine-readable data
   - `SLOTS.md` - Human-readable Markdown
   - `index.html` - GitHub Pages ready HTML
   - `images/` and `thumbnails/` - Slot visualizations (when downloaded)
