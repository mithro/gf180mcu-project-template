...
// on the kept side of a half-size die, each pinned (via a custom PAD_CFG)
// to its EXACT slot_1x1 (x,y).  slot_1x1's analog pads sit on the cut-away
// NORTH corner and cannot be coordinate-preserved, so these slots have NO
// analog pads: each defines NO_ANALOG_PADS to drop the core-unused analog
// port (so its width never collapses to [-1:0]).
